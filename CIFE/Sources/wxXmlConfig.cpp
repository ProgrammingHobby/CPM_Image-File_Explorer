...
bool wxXmlConfig::Flush(bool bCurrentOnly) {
    ...
    if (!m_strGlobalFile &&
            (GetStyle() & (wxCONFIG_USE_GLOBAL_FILE | wxCONFIG_USE_LOCAL_FILE))) {
        if (!wxFile::Exists(m_strLocalFile)) {
            wxFileName fileName(m_strLocalFile);

            if (fileName.GetDirCount() && !fileName.Mkdir()) {
                wxLogError(_("can't create user configuration directory."));
                return false;
            }
        }
        ...
        if (!wxFile::Exists(m_strGlobalFile)) {
            wxFileName fileName(m_strGlobalFile);

            if (fileName.GetDirCount() && !fileName.Mkdir()) {
                wxLogError(_("can't create global configuration directory."));
                return false;
            }
        }

        if (!m_xmlDoc->Save(m_strGlobalFile)) {
            wxLogError(_("can't save global configuration file."));
            return false;
        }
    }
...
wxString wxXmlConfig::GetGlobalDir() {
    return wxStandardPaths::Get().GetLocalDataDir() << wxT("/");
}

// --------------------------------------------------------------------------------
wxString wxXmlConfig::GetLocalDir() {
    return wxStandardPaths::Get().GetUserLocalDataDir() << wxT("/");
}
...
wxXmlConfigEntry *wxXmlConfig::AddEntry(const wxString &key) {
    ...
    wxXmlConfigEntry *newEntry = new wxXmlConfigEntry(pParent, wxXML_ELEMENT_NODE, key,
            wxEmptyString, NULL, pNext);
...
// --------------------------------------------------------------------------------
const wxString &wxXmlConfig::GetPath() const {
    return m_strPath;
}
...
wxXmlConfigGroup *wxXmlConfig::AddGroup(const wxString &key) {
    ...
    // create the new entry
    wxXmlConfigGroup *newGroup = new wxXmlConfigGroup(pParent, wxXML_ELEMENT_NODE, key,
            wxEmptyString, NULL, pNext);
